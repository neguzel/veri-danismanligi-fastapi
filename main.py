...
def render_chart_from_spec(df: pd.DataFrame, upload_id: int, spec: Dict[str, Any]) -> Optional[str]:
    """
    OpenAI'den gelen grafik spec'ine göre matplotlib ile grafik üretir.
    - title, type, x, y, columns vs. alanlarını okur
    - Dosyayı CHART_DIR altına kaydedip /static/charts/... URL'sini döner
    """
    chart_type = (spec.get("type") or "line").lower()
    ...
    try:
        ...
        if chart_type in ("scatter", "point"):
            if x and y and x in df.columns and y in df.columns:
                ...
                # Başlıkta "ortalama" geçiyorsa: gerçekten ortalama çizelim
                if "ortalama" in title.lower():
                    grouped = (
                        data.groupby(x)[y]
                        .mean()
                        .reset_index()
                    ...
                    # Ham scatter: küçük ve saydam noktalar
                    ax.scatter(data[x], data[y], s=6, alpha=0.25)
        ...
        # ---- 5) BOX PLOT ----
            ...
            # Tanınmayan grafik tipi: hiç üretme
            plt.close(fig)
            return None
        ...
        safe_title = safe_filename_part(title.replace(" ", "_")) or "chart"
        filename = f"{upload_id}_{safe_title}.png"
